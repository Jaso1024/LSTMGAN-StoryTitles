    ...
    def __init__(self, text, maxlen) -> None:
        ...
        lookup_table = StaticVocabularyTable(
            KeyValueTensorInitializer(
                keys=self.vocab,
                key_dtype=tf.string,
                values=tf.range(0, len(self.vocab), dtype=tf.int64)
            ), 
            num_oov_buckets=100
        )
...
class Discriminator(Model):
    def __init__(self, num_uniques):
        ...
        self.t3 = GRU(256)
        self.t3 = Dense(256, activation="relu")
        self.t4 = Flatten()
...
class GAN(Model):
    ...
    @tf.function
    def trainstep(self, fake_group, real_group, real_text, noise):
        
        with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
            fake_text = self.generator([fake_group, noise], training=True)
        ...
        generator_grad = tape1.gradient(generator_loss, self.generator.trainable_variables)
    ...
    def train(self, fake_groups, real_groups, text_data, noises, epochs, ckpt_freq=150):
